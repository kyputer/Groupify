...
async function downvote(SpotifyID, userID, playlistID = 1) {
  console.log(
    `Downvoting track: ${SpotifyID} by user: ${userID} in playlist: ${playlistID}`
  );
  let conn;
  try {
    conn = await getDBConnection();
...
var pushBlacklist = async function (trackID) {
  ...
  c.query(
    "UPDATE track SET blacklist=blacklist+1 WHERE blacklist IS NOT NULL;"
  );
...
async function getNext(index, callback) {
  ...
  try {
    ...
    await c.query(
      "SELECT * FROM tracks WHERE blacklist IS NULL ORDER BY votes DESC;",
      function (err, rows) {
        if (err) throw err;
...
async function getPlayed(callback) {
  ...
  try {
    conn = await getDBConnection();
    const rows = await conn.query(
      "SELECT * FROM tracks WHERE blacklist IS NOT NULL ORDER BY Votes DESC;"
    );
